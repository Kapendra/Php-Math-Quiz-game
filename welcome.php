...
<body>
    <nav class="navbar navbar-default title1">
       ...
       <!-- In this section we can see that the API of Intresting Number trivia and the Score and the History of the marks which are fetched thorugh the
          mysql database  -->
        </div>
    </div>
    </nav>
        ...
        <div class="row">
            <div class="col-md-12">
                <?php if(@$_GET['q']==1) 
                {
                    ...
                    $jsonurl = "http://numbersapi.com/random?json";
                    $json = file_get_contents($jsonurl);
                    $data = json_decode($json);
                    $trivia = $data->text;

                    echo '  <div class="col-md-12 bg-primary fact-card p-2"><div class="container-fluid px-4"><h1 class="display-5 fw-bold">Interesting Number Trivia</h1><p class="col-md-12 fs-4">'.$trivia.'</p><br><button class="btn btn-primary btn-lg" type="button" style="margin-bottom:30px" onclick="window.location.reload(true)">More Fact</button><br></div></div>';
                }?>
                ...
                ?>
 <!-- In this section we can see that the history of the user what he has done recently and other side the ranking system 
where the users are compared from the database and winner is on the top with highest marks and loser on the bottom with lowest marks  -->
                <?php
                    if(@$_GET['q']== 2) 
                    {
